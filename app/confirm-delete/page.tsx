    ...
    <>
      ...
      {status === "success" && (
        <>
          ...
          <p className="text-slate-500 text-sm">Toutes vos données ont été effacées. Redirection...</p>
        </>
      )}
      ...
      {status === "error" && (
        <>
          ...
          <a href="/dashboard/agent" className="inline-block text-xs text-slate-500 hover:text-white transition-colors mt-2">
            Retour au dashboard
          </a>
        </>
      )}
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] flex items-center justify-center p-6">
      <div className="bg-[#13131A] border border-white/5 rounded-2xl p-8 max-w-sm w-full text-center space-y-4">
        ...
        <Suspense fallback={<Loader2 size={28} className="animate-spin text-slate-400 mx-auto" />}>
